...
import com.driving.notification.entity.Message;
...
public class MessageMapper {
    ...
    public List<HashMap> searchMessageByPage(long userId, String identity, long start, int length) {
        ...
        Aggregation aggregation = Aggregation.newAggregation(Aggregation.addFields().addField("id").withValue(json).build(), Aggregation.lookup("message_ref", "id", "messageId", "ref"), Aggregation.match(Criteria.where("ref.receiverId").is(userId).and("ref.receiverIdentity").is(identity)), Aggregation.sort(Sort.by(Sort.Direction.DESC, "sendTime")), Aggregation.skip(start), Aggregation.limit(length));
