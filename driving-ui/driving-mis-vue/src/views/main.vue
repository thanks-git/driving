		...
		<nav class="site-navbar" :class="'site-navbar--' + navbarLayoutType">
			...
			<div class="site-navbar__body clearfix">
				<el-menu class="site-navbar__menu" mode="horizontal">
					<el-menu-item class="site-navbar__switch" index="0" @click="handleSwitch"><SvgIcon name="zhedie" class="icon-svg" /></el-menu-item>
				</el-menu>
			...
			<div class="site-sidebar__inner">
				<el-menu :default-active="menuActiveName || 'home'" :collapse="sidebarFold" :collapseTransition="false" class="site-sidebar__menu">
					...
					<el-submenu index="组织管理" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'">
						...
						<el-menu-item index="dept" v-if="isAuth(['ROOT', 'DEPT:SELECT'])" @click="$router.push({ name: 'Dept' })" ref="ABC">
							<SvgIcon name="company_fill" class="icon-svg" />
							<span slot="title">部门管理</span>
						</el-menu-item>
						<el-menu-item index="role" v-if="isAuth(['ROOT', 'ROLE:SELECT'])" @click="$router.push({ name: 'Role' })">
							<SvgIcon name="role_fill" class="icon-svg" />
							<span slot="title">角色管理</span>
						</el-menu-item>
						<el-menu-item index="user" v-if="isAuth(['ROOT', 'USER:SELECT'])" @click="$router.push({ name: 'User' })">
					...
					<el-submenu index="司机管理" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'">
						...
						<el-menu-item index="driver" v-if="isAuth(['ROOT', 'DRIVER:SELECT'])" @click="$router.push({ name: 'Driver' })">
							<SvgIcon name="company_fill" class="icon-svg" />
							<span slot="title">司机信息</span>
						</el-menu-item>
						<el-menu-item index="driver_wallet" v-if="isAuth(['ROOT', 'DRIVER_WALLET:SELECT'])" @click="$router.push({ name: 'DriverWallet' })">
							<SvgIcon name="company_fill" class="icon-svg" />
							<span slot="title">司机钱包</span>
						</el-menu-item>
						<el-menu-item index="driver_fine" v-if="isAuth(['ROOT', 'DRIVER_FINE:SELECT'])" @click="$router.push({ name: 'DriverFine' })">
							<SvgIcon name="role_fill" class="icon-svg" />
							<span slot="title">司机罚款</span>
						</el-menu-item>
						<el-menu-item index="user" v-if="isAuth(['ROOT', 'DRIVER_LOCKDOWN:SELECT'])" @click="$router.push({ name: 'DriverLockdown' })">
							<SvgIcon name="user_fill" class="icon-svg" />
					...
					<el-submenu index="运营管理" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'">
						...
						<el-menu-item index="order" v-if="isAuth(['ROOT', 'ORDER:SELECT'])" @click="$router.push({ name: 'Order' })">
							<SvgIcon name="company_fill" class="icon-svg" />
							<span slot="title">订单信息</span>
						</el-menu-item>
						<el-menu-item index="monitoring" v-if="isAuth(['ROOT', 'MONITORING:SELECT'])" @click="$router.push({ name: 'Monitoring' })">
							<SvgIcon name="company_fill" class="icon-svg" />
					...
					<el-submenu index="规则管理" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'">
						...
						<el-menu-item index="approval" v-if="isAuth(['ROOT', 'WORKFLOW:APPROVAL', 'FILE:ARCHIVE'])" @click="$router.push({ name: 'Approval' })">
	...
	methods: {
		...
		loadSiteContentViewHeight: function() {
			...
			if (this.$route.meta.isTab) {
				height -= 40;
				this.siteContentViewHeight = isURL(this.$route.meta.iframeUrl) ? { height: height + 'px' } : { minHeight: height + 'px' };
